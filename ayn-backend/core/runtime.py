...
VALID_APP_ROLES = {"web", "realtime", "scheduler", "worker", "migrate", "all"}
...
def should_enable_workers(role: str, flag_enabled: bool) -> bool:
    return role in {"worker", "all"} and flag_enabled
...
def validate_environment(cfg: RuntimeConfig) -> None:
    ...
    if missing and cfg.strict_env_validation:
        if cfg.app_env == "production" and "REDIS_URL" in missing:
            raise RuntimeError(
                "Missing required environment variables: "
                f"{', '.join(missing)}. Redis is required in production; "
                "deployment will fail until REDIS_URL is configured."
            )

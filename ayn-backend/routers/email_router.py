...
async def send_ticket_reply(req: TicketReplyRequest, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        asyncio.create_task(send_email(req.user_email, "ticket_reply", {
            "content": req.content, "ticket_id": req.ticket_id
        }))
        return {"ok": True}
    except Exception as e:
...
async def send_reply_email(req: ReplyEmailRequest, user: dict = Depends(get_current_user)):
    ...
    try:
        asyncio.create_task(send_email(req.email, "application_reply", {
            "content": req.content, "application_id": req.application_id
        }))
        return {"ok": True}
    except Exception as e:
        return {"ok": False, "error": str(e)}

...
flowchart TB
    subgraph client["Frontend (React/Vite)"]
        ...
        ext["Chrome extension — device token"]
...
| `resume-hub`'s auth handling (the dispatcher's entry checks) | both auth lanes explicitly: session JWT (web) and device token (extension) | These are two genuinely different mechanisms sharing one dispatcher. A new check written and tested only against a real web session can silently never fire for the extension's device-token calls (or the reverse) — "I tested it signed in on the website" does not prove it works for the extension, and vice versa. |
...
13. **Touching the extension: bump the version in both places, then rebuild.** `manifest.json`'s version and `content.js`'s `AYN_BUILD` fallback constant both need to move together, then `node extension/build.mjs` regenerates `public/ayn-extension.zip` and `public/ayn-extension-version.json` — never hand-edit those two generated files directly. If the change affects what the extension sends or expects from `resume-hub`, also consider whether `system_config.extension_min_version` needs raising so older, incompatible sideloaded copies get the real "please update" (426 `extension_outdated`) response instead of a confusing failure.
...
8. **The extension's view of a profile is a cache, not a live read — new fields need to invalidate it.** The sidepanel keeps its own 24-hour TTL identity cache, cleared only by a specific `AYN_PROFILE_UPDATED` signal fired on real profile saves. A new profile field that the extension's scoring or tailoring should actually see needs to confirm that save path fires this same signal — otherwise the field is invisible to the extension for up to a day after the user fills it in, with no error anywhere to explain why.

9. **Destructive actions get an explicit confirm; recovery actions don't — keep that asymmetry.** Delete Account requires a typed-email match; admin kill switches require a named confirm only on the OFF direction, never on ON, because turning something back on during an incident shouldn't be slowed down. A new destructive, hard-to-reverse action should follow the same shape (confirm the dangerous direction, don't gate the safe/recovering one) rather than either no confirmation or over-guarding something reversible.
...
11. **Multi-step forms need to persist *where* the user is, not just *what* they've entered.** The employer intake wizard encodes its current step inside the same `phase` column it already uses for status (`"asking:<step>"`) so a reload resumes at the actual question left off, not the first one — this was broken once by an early-return bug that silently dropped the step. A new multi-step flow should follow the same "resume at the real position" discipline, not just save field values and reset to step one.
...
- **The extension is read-only, permanently** (`CLAUDE.md`'s own global rule #4). Never add code that writes to, clicks, or types into a third-party page from extension code, regardless of what feature is being built.

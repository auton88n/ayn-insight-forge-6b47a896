...
Extension lane (x-ayn-ext-token): ext_bootstrap, ext_ingest_job, ext_job_ingest, ext_profile (compact fact vector; EXCLUDES work-auth/EEO/salary by design, those stay AI-side), ext_autofill, ext_log_result, ext_vision_fill, ext_tailor, ext_cover_letter, ext_cover_letter_text, ext_job_score, ext_suggest_roles, ext_find_contacts, ext_save_application, ext_get_applications, ext_update_application, ext_download_resume_text, ext_ask, smart_tailor, ext_save_answer, ext_lookup_answer, ext_get_resume_blob, ext_profile_canonical_get.
...
| AUTO_TRACK_SUBMIT enrichment | sends title/company/url only | PARTIAL: no match_score or job_id at capture time. |

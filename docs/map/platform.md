...
CONSENT RECORD. src/lib/legal.ts is the single source of the document versions: LEGAL.termsVersion 1.0, LEGAL.privacyVersion 1.0, effective 1 August 2026. The signup checkbox in AuthModal reads them, and recordLegalConsent() posts the resume-hub action legal_consent_record after signUp succeeds, best effort and never blocking. terms_consent_log gained privacy_version, ip_address and source, the IP being read server side from x-forwarded-for because the browser cannot know it. The table is append only, so a materially changed version later is a new row and an unaccepted version shows as a missing one. The reprompt flow itself is deliberately not built yet. get_admin_account_detail now returns legal_consent (the latest row) and legal_consent_history (up to ten), rendered as a Legal consent card in src/components/admin/sections/system/AccountDetail.tsx.


## Legal documents (v3.32.0)

...
Last verified: v3.5.0 a profile that can actually be matched. Profile is now FIVE collapsible groups in this order: Your resume, About you, Your experience, What you are looking for, Work eligibility. Skills are objects with level, years and last used, not bare strings (old strings migrate to level null). Work history rows show title, company, dates and location by default and expand to industry, team size and 2 to 5 achievement bullets, which are what tailoring rewrites. Education carries field of study. New fields: what you are known for, employment type, availability, company stage, work permit expiry. Provenance text only appears when informative ("From your resume", or "Edited by you" with revert). A matching-readiness line at the top names the one or two highest-impact gaps, never a percentage. Autosave on blur, no Save button. buildProfileText and candidate_skills (new level, years, last_used columns) now carry these signals so employers can match on them. Still one profile, one resume: BuilderTab.tsx is gone, tailored documents are outputs downloaded from JobsTab. July 2026.

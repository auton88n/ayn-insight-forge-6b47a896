...
Last verified: v3.5.1 honest discovery consent. The Get discovered tab no longer claims an "anonymized profile": copy now states plainly that employers searching AYN can see the full profile (resume, work history, skills, education, preferences, eligibility) and that AYN's AI matches on all of it, with email and phone shared only after an approved intro. Turning discovery ON opens a confirmation dialog before anything is written, and talent_pool_set records consent_version (new column on talent_pool_consent, current value v3.5.1-full-profile). Landing "Be found, not just seen" card and Trust chips match the same promise. No matching, indexing, or reveal-flow behaviour changed. Preceded by v3.5.0 "a profile that can actually be matched": Profile is FIVE collapsible groups (Your resume, About you, Your experience, What you are looking for, Work eligibility); skills are objects with level, years and last used; work history rows show title, company, dates and location and expand to industry, team size and 2 to 5 achievement bullets; education carries field of study; new fields what you are known for, employment type, availability, company stage, work permit expiry; provenance text only when informative; matching-readiness line instead of a percentage; autosave on blur; buildProfileText and candidate_skills carry the new signals. Still one profile, one resume: BuilderTab.tsx is gone, tailored documents are outputs downloaded from JobsTab. July 2026.

...
Last verified: commit "v2.11.1 in-page presence (activity glow recolored to AYN brand orange with rgba literals inlined because the style is injected into arbitrary pages; new floating orange FAB, 48px circle expanding to a 'Fill with AYN' pill on hover, injected only on top-frame 'apply' pages via classifyPage, never on aynn.io, isolated in a closed shadow root attached to #__ayn_fab_host__; clicking sends a new OPEN_SIDE_PANEL message handled in background.js which calls chrome.sidePanel.open on the user gesture and broadcasts FOCUS_SIDEPANEL_TAB; sidepanel also honors a short-lived ayn_focus_tab flag in chrome.storage.local for the first-open race; master toggle 'Show the floating button on application pages' on the Fill tab stored as ayn_fab_enabled, per-host 7-day dismissals stored as ayn_fab_dismissed; check-wiring updated for OPEN_SIDE_PANEL)", manifest v2.11.1, AYN_BUILD 2.11.1, July 28 2026.

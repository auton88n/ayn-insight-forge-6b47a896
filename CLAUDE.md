...
Last verified: commit "v2.12.0 one lane, real documents (Part 1: extension/question-engine/learning/supabase-store.ts now runs a bounded local fuzzy pass after exact question_signature misses — fetches the user's top-200 ext_answer_memory rows ONCE per fill session, tokenizes with stopword-strip, scores containment with score ≥ 0.7 AND ≥ 3 overlapping content tokens, only compares rows with the same question_kind, hard-excludes sensitive questions via AYN_SENSITIVE_NO_GUESS semantic types plus the v2.10.1 regex so work-auth / sponsorship / EEO / salary / notice-period stay exact-signature only; fuzzy hits surface as Answer.reasoning 'Reused your answer to: <source>' so the sidepanel notes the reuse; exact-signature path unchanged in speed and behaviour. Part 2: five dead EXT_ACTIONS deleted from supabase/functions/resume-hub/index.ts — ext_tailor, ext_cover_letter, ext_job_ingest, ext_save_answer, ext_lookup_answer; live twins are smart_tailor and ext_cover_letter_text; ext_job_ingest cache logic already lives inline in ext_job_score; ext_save_answer/ext_lookup_answer superseded by the direct PostgREST learning store. Part 3: symmetric exports — tailor tab gets a Download PDF button (buildResumePdfBlob) beside DOCX; cover tab gets a Download Word button (new buildCoverLetterDocxBlob using docx UMD) beside PDF; all four exports use jsPDF text calls or the docx UMD — no html2canvas / snapdom / rasterization anywhere in the resume or cover path, single column, Times New Roman, no tables / headers / footers / images, so PDF text stays selectable and ATS-parseable; filenames standardized to <FirstLast>-<Company>-<Role>.<ext> via aynFileName (strips illegal chars, collapses to hyphens, caps at 80). Part 4: scripts/check-wiring.mjs adds a REVERSE check (every EXT_ACTIONS entry must be referenced by at least one caller in extension/ or src/, else fail) and a WARN-only report of direct PostgREST table access from extension/ so the learning-store's direct lane stays visible; AYN_BUILD fallback bumped to 2.12.0)", manifest v2.12.0, AYN_BUILD 2.12.0, July 29 2026.

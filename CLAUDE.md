...
Last verified: commit "v2.11.4 gap-aware tailoring (ext_tailor + smart_tailor accept optional matched_skills/missing_skills from the scorer and get one added prompt rule to surface missing skills only when the resume/canonicalProfile really supports them, plus an explicit metric-preservation rule so numbers, percentages, dollar figures, headcount, timeframes, dates, and job titles must appear in the output exactly as in the input; ext_cover_letter + ext_cover_letter_text accept optional length ('short'|'standard'|'detailed' -> 180/280/400 word caps) and free-text guidance (max 200 chars, honoured only when supported); sidepanel caches SJ.matchedSkills/SJ.missingSkills off the ext_job_score response and threads them into both smart_tailor call sites; Cover tab gets a Length select and an emphasis input above Generate; AYN_BUILD fallback bumped to 2.11.4)", manifest v2.11.4, AYN_BUILD 2.11.4, July 28 2026.

...
Last verified: commit "v3.2.1 reindex actually wired. Part 4 of v3.2.0 shipped only partially: every client-side write that changes indexed content now routes through the single helper src/lib/talentPoolSync.ts reindexTalentPool(reason) (opt-in gated, coalesced, fire and forget, swallows errors, dispatches AYN_POOL_REINDEXED so a mounted TalentPoolCard refreshes its freshness line). Seven call sites: BuilderTab save, setPrimary, removeResume, handleFileParsed, aiImprove; ProfileTab handleResumeParsed and save. Production audit: candidate_index is empty, so there are zero deterministic-v1 rows to re-index. Preceded by v3.2.0 one profile, and show what it powers. The Hub now shows exactly ONE profile: ProfileTab.tsx merges the old Profile and Canonical surfaces, mirrors _shared/identity.ts precedence (profile > canonical > resume > account) with a per-field source label, and always writes edits to the user-entered layer. Fields are grouped by what they power: About you, What you are looking for, Work eligibility, Your experience. CanadianProfileForm.tsx deleted. New TalentPoolCard.tsx renders the anonymized card employers actually see, splits skills into resume-backed versus AYN-inferred with delete on inferred, shows index freshness with a Refresh action, and nudges on the two fields employers filter on. New backend action talent_pool_skill_delete; talent_pool_get now returns preview, skills with provenance, and freshness timestamps. Stale index fixed: talent_pool_reindex_self now fires after every client-side write (resume upload, resume save, primary change, profile save). No extension changes in this release.", manifest v3.1.0, AYN_BUILD 3.1.0, August 2026.

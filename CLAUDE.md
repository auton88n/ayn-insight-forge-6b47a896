...
Last verified: commit "v2.12.1 one page, always readable — PDF builders in extension/resumeFormat.js run a two-pass MEASURE-then-DRAW layout with a bounded auto-fit loop. Resume style scale has 7 steps from top (identical to v2.12.0 output, body 10 / section 11 / name 20 / LH 1.25 / margin 54) to floor (body 9 / section 10 / name 16 / LH 1.12 / margin 40.5 / spacing 0.6x). Cover scale steps body 11→10 and LH 1.45→1.25 with margins fixed at 72pt. Fit loop picks the first step that fits one page; if the floor still overflows, the builder does NOT shrink further and does NOT silently paginate — it renders two pages and returns { fitsOnePage:false, overflowLines, longestSections:[top 2 by measured height] }. Sidepanel surfaces the report as one muted line ('This resume needs about <n> more lines than one page. Trim <A> or <B> to fit.') under the Download button via #fill-fit-hint / #tailor-fit-hint / #cover-fit-hint. ATS-safe constraints: only font size, line height, margins, and spacing change; never multiple columns, tables, images, or rasterization; content never dropped. PDF builders now return { blob, fitsOnePage, overflowLines, longestSections } and callers unpack .blob before saving; DOCX builders unchanged. New scripts/validate-pdf.mjs loads the real browser builders in a Node vm sandbox (jspdf.umd.min.js + docx.min.js + resumeFormat.js), generates resume and cover PDFs from fixtures, parses each with @opendataloader/pdf (Apache-2.0, needs Java 11+), and asserts (a) real extracted text no OCR, (b) exactly 1 page, (c) name before first section + section order preserved, (d) every input section present in parsed output. Wired as OPTIONAL npm run validate:pdf and deliberately NOT called from extension/build.mjs since Java may be unavailable at build time. AYN_BUILD fallback bumped to 2.12.1", manifest v2.12.1, AYN_BUILD 2.12.1, July 28 2026.

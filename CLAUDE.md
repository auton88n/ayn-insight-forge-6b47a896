...
Last verified: v3.7.0 the dashboard chat becomes a job search copilot. The last general purpose surface in the product is gone. ayn-unified was roughly 1500 lines of market intelligence: it scanned crypto pairs, pulled commodity prices, built country profiles, ran an autonomous paper trading loop, and branched into structural engineering. A job seeker asking what to do next got told about gold and oil. All of it is deleted, along with the engineering intent, EngineeringContext, calculatorType and buildingCode. The function is now four small files: index.ts (auth, 100 message per chat cap, plan limit, SSE passthrough), jobContext.ts (grounding), systemPrompts.ts (one prompt, not thirty four kilobytes of persona), llmGateway.ts (chains cut to chat, deep, files). The copilot reads the person's own identity, profile, skills with level and recency, work history with achievements, preferences, work eligibility, saved jobs with their newest match score and breakdown, discovery status, pending proposals and remembered facts, all server side under the service role, and it never loads email, phone or address into the prompt. When something is missing it says so and names the Resume Hub tab. An empty chat opens with three prompts built from real state, not "Tell me more". The extension's ext_ask stays a separate prompt on purpose: one job versus one career. July 2026.

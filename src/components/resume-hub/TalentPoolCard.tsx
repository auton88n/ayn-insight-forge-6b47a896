 ...
 *
 * v3.69.0: the on/off switch, badge, and restriction message all moved into
 * ProfileTab (that card renders unconditionally; this one renders nothing
 * until opted in). This component is now the body only — no header, no
 * toggle, nothing to duplicate what the card above it already says.
 */
...
import { Loader2, RefreshCw, X, ShieldCheck, Check, AlertCircle } from "lucide-react";
...
export default function TalentPoolCard({ refreshKey = 0, groupGaps }: Props) {
  ...
  // Nothing to show until we know the seeker is opted in — the on/off state
  // itself is ProfileTab's card, not this one.

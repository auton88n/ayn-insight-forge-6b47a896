 ...
 * Pure CSS: black backgrounds, gold gradients, ambient glows, geometric elements.
 */

import { memo } from 'react';
import { motion } from 'framer-motion';
import { Link } from 'react-router-dom';
import { BarChart3, Target, Search, Globe, Zap, Shield } from 'lucide-react';
  ...
  return (
    <div style={{ background: '#000' }}>
      ...
      {/* ══════════════════════════════════════════════════════════
          SECTION 1 — MEET AYN
      ══════════════════════════════════════════════════════════ */}
      <section style={{ position: 'relative', minHeight: '100dvh', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '80px clamp(32px,6vw,96px)', overflow: 'hidden', background: '#000' }}>

        {/* Ambient gold glow — right side where robot would be */}
        <div style={{ position: 'absolute', top: '50%', right: '10%', transform: 'translateY(-50%)', width: 640, height: 640, borderRadius: '50%', background: 'radial-gradient(circle, rgba(226,183,105,0.10) 0%, rgba(226,183,105,0.04) 45%, transparent 70%)', pointerEvents: 'none', zIndex: 0 }} />
        {/* Subtle grid */}
        ...
        {/* Floating geometric rings — right side */}
        <div style={{ position: 'absolute', right: '15%', top: '50%', transform: 'translateY(-50%)', zIndex: 1, pointerEvents: 'none' }}>
          {[400, 300, 200, 120].map((size, i) => (
            <div key={i} style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: size, height: size, borderRadius: '50%', border: `1px solid rgba(226,183,105,${0.04 + i * 0.03})`, }} />
          ))}
          {/* Center dot */}
        ...
        <div style={{ position: 'relative', zIndex: 10, maxWidth: 580 }}>
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', borderRadius: 100, border: `1px solid rgba(226,183,105,0.25)`, background: 'rgba(226,183,105,0.06)', marginBottom: 28 }}>
          ...
          <motion.h1 initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.8, ease: 'easeOut' }}
            style={{ fontFamily: FB, fontSize: 'clamp(72px,10vw,130px)', fontWeight: 400, lineHeight: 0.88, letterSpacing: '-0.01em', color: '#fff', margin: '0 0 28px' }}>
            {isAr ? 'تعرّف على ' : 'MEET '}
          ...
          <motion.p initial={{ x: -30, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 0.2 }}
            style={{ fontFamily: F, fontSize: 17, fontWeight: 300, lineHeight: 1.7, color: 'rgba(255,255,255,0.52)', maxWidth: 400, margin: '0 0 44px' }}>
          ...
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 0.4 }}
            ...
            <Link to="/features" style={{ display: 'inline-flex', alignItems: 'center', padding: '14px 36px', borderRadius: 100, fontFamily: F, fontSize: 15, fontWeight: 500, color: '#fff', border: '1px solid rgba(255,255,255,0.22)', textDecoration: 'none', transition: 'all 0.2s' }}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8, duration: 0.8 }}
            style={{ display: 'flex', gap: 40, marginTop: 64, paddingTop: 40, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            {[{ n: '187+', l: 'Countries' }, { n: '73', l: 'AI Agents' }, { n: '24/7', l: 'Monitoring' }].map((s, i) => (
              <div key={i}>
                ...
                <p style={{ fontFamily: F, fontSize: 11, letterSpacing: '0.18em', color: 'rgba(255,255,255,0.30)', textTransform: 'uppercase', margin: 0 }}>{s.l}</p>
              </div>
            ))}
          </motion.div>
        </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 2 }}
          ...
          <span style={{ fontFamily: F, fontSize: 10, letterSpacing: '0.4em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.28)', fontWeight: 600 }}>Scroll to explore</span>
          <div style={{ width: 1, height: 64, background: 'linear-gradient(to bottom, rgba(226,183,105,0.5), transparent)' }} />
        </motion.div>
      </section>
      ...
      ══════════════════════════════════════════════════════════ */}
      <section style={{ position: 'relative', minHeight: '100dvh', display: 'flex', alignItems: 'center', padding: '0 clamp(32px,6vw,96px)', overflow: 'hidden', background: '#030200' }}>
        {/* Large ambient glow */}
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 80% 60% at 70% 50%, rgba(226,183,105,0.07) 0%, transparent 70%)', pointerEvents: 'none' }} />
        {/* Diagonal lines */}
        <div style={{ position: 'absolute', inset: 0, background: 'repeating-linear-gradient(135deg, transparent, transparent 60px, rgba(226,183,105,0.015) 60px, rgba(226,183,105,0.015) 61px)', pointerEvents: 'none' }} />

        {/* Right: abstract geometry */}
        <div style={{ position: 'absolute', right: '8%', top: '50%', transform: 'translateY(-50%)', width: 480, height: 480, pointerEvents: 'none' }}>
          {/* Hexagon-like shape */}
          <motion.div animate={{ rotate: [0, 360] }} transition={{ duration: 30, repeat: Infinity, ease: 'linear' }}
            style={{ position: 'absolute', inset: 0, border: '1px solid rgba(226,183,105,0.12)', borderRadius: '30% 70% 70% 30% / 30% 30% 70% 70%' }} />
          <motion.div animate={{ rotate: [360, 0] }} transition={{ duration: 20, repeat: Infinity, ease: 'linear' }}
            style={{ position: 'absolute', inset: 40, border: '1px solid rgba(226,183,105,0.08)', borderRadius: '70% 30% 30% 70% / 70% 70% 30% 30%' }} />
          {/* Center glow */}
          <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 160, height: 160, borderRadius: '50%', background: 'radial-gradient(circle, rgba(226,183,105,0.18) 0%, transparent 70%)' }} />
          {/* Data points */}
        ...
        <div style={{ position: 'relative', zIndex: 10, maxWidth: 520 }}>
          ...
          <motion.h2 initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}
            style={{ fontFamily: F, fontSize: 'clamp(48px,7vw,100px)', fontWeight: 700, lineHeight: 1.04, letterSpacing: '-0.025em', color: '#fff', margin: '0 0 28px' }}>
            {isAr ? 'ذكاء\nمتطوّر.' : 'Intelligence,\nevolved.'}
          </motion.h2>
          <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2, duration: 0.8 }}
            style={{ fontFamily: F, fontSize: 16, fontWeight: 300, lineHeight: 1.75, color: 'rgba(255,255,255,0.45)', maxWidth: 420 }}>
      ...
      ══════════════════════════════════════════════════════════ */}
      <section style={{ position: 'relative', minHeight: '100dvh', display: 'flex', alignItems: 'center', padding: '80px clamp(32px,5vw,80px)', overflow: 'hidden', background: '#020201' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 60% 60% at 30% 50%, rgba(226,183,105,0.06) 0%, transparent 65%)', pointerEvents: 'none' }} />

        <div style={{ position: 'relative', zIndex: 10, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center', width: '100%', maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div>
            ...
            <motion.h2 initial={{ opacity: 0, scale: 0.92 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }}
              style={{ fontFamily: F, fontSize: 'clamp(38px,5vw,72px)', fontWeight: 700, lineHeight: 1.1, color: '#fff', letterSpacing: '-0.02em', margin: '0 0 24px' }}>
              {isAr ? 'أعمالك،\nمُفهومة.' : 'Your business,\nunderstood.'}
            </motion.h2>
            <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.2 }}
              style={{ fontFamily: F, fontSize: 15, fontWeight: 300, lineHeight: 1.75, color: 'rgba(255,255,255,0.42)', maxWidth: 380 }}>
          ...
          <div style={{ position: 'relative', height: 440 }}>
            ...
            <motion.div initial={{ opacity: 0, x: 40, y: 30 }} whileInView={{ opacity: 1, x: 0, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.2 }}
              ...
              <div style={{ height: 72, display: 'flex', alignItems: 'flex-end', gap: 3 }}>
                {[35, 65, 40, 88, 60, 78, 45, 70, 55].map((h, i) => (
                  <div key={i} style={{ flex: 1, height: `${h}%`, background: `linear-gradient(to top, rgba(226,183,105,0.2), rgba(226,183,105,0.65))`, borderRadius: '3px 3px 0 0' }} />
                ))}
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 60, y: 80 }} whileInView={{ opacity: 1, x: 0, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.4 }}
              ...
              <div style={{ width: '100%', height: 3, background: 'rgba(255,255,255,0.08)', borderRadius: 2, overflow: 'hidden' }}>
                <motion.div initial={{ width: 0 }} whileInView={{ width: '84%' }} viewport={{ once: true }} transition={{ duration: 1.2, delay: 0.6 }}
                  style={{ height: '100%', background: `linear-gradient(to right, ${G}, #a07830)`, borderRadius: 2 }} />
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: -20, y: 60 }} whileInView={{ opacity: 1, x: 0, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.55 }}
              ...
              <p style={{ fontFamily: F, fontSize: 10, color: 'rgba(255,255,255,0.40)', letterSpacing: '0.16em', textTransform: 'uppercase', margin: 0 }}>Countries</p>
      ...
      <section style={{ position: 'relative', minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px clamp(24px,4vw,64px)', overflow: 'hidden', background: '#050505' }}>
        {/* Central glow */}
        <div style={{ position: 'absolute', top: '30%', left: '50%', transform: 'translate(-50%,-50%)', width: 500, height: 500, borderRadius: '50%', background: 'radial-gradient(circle, rgba(226,183,105,0.08) 0%, transparent 65%)', pointerEvents: 'none' }} />

        <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
          style={{ fontFamily: F, fontSize: 10, fontWeight: 600, letterSpacing: '0.30em', textTransform: 'uppercase', color: G, margin: '0 0 16px', textAlign: 'center' }}>Services</motion.p>
        <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          style={{ fontFamily: FB, fontSize: 'clamp(36px,5vw,72px)', color: '#fff', margin: '0 0 64px', textAlign: 'center', fontWeight: 400 }}>
          {isAr ? 'ما يفعله عين' : 'What AYN Does'}
        </motion.h2>

        {/* 3 cards */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20, width: '100%', maxWidth: 1100, marginBottom: 72 }}>
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              ...
              style={{ padding: '32px 28px', borderRadius: 32, border: card.active ? '1px solid rgba(226,183,105,0.28)' : '1px solid rgba(255,255,255,0.06)', background: card.active ? undefined : 'transparent', boxShadow: card.active ? '0 0 50px rgba(212,160,23,0.10)' : 'none', transition: 'all 0.3s', cursor: 'default' }}
              ...
              <div style={{ width: 52, height: 52, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 28, background: card.active ? G : 'rgba(255,255,255,0.06)' }}>
                <card.icon size={22} color={card.active ? '#000' : 'rgba(255,255,255,0.38)'} />
              </div>
              <h3 style={{ fontFamily: FB, fontSize: 24, color: '#fff', margin: '0 0 12px', letterSpacing: '0.02em' }}>{card.title}</h3>
              <p style={{ fontFamily: F, fontSize: 14, color: 'rgba(255,255,255,0.38)', lineHeight: 1.65, margin: 0 }}>{card.desc}</p>
            </motion.div>
          ))}
        </div>

        {/* ASK → ANALYZE → EXECUTE */}
        <div style={{ width: '100%', maxWidth: 680, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative' }}>
          <div style={{ position: 'absolute', top: 22, left: '15%', right: '15%', height: 1, background: 'linear-gradient(to right, transparent, rgba(255,255,255,0.15), transparent)' }} />
          {[{ label: 'ASK', icon: Search }, { label: 'ANALYZE', icon: BarChart3 }, { label: 'EXECUTE', icon: Target }].map((step, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }}
              ...
              <div className="stitch-glass" style={{ width: 52, height: 52, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.3s' }}
                onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.borderColor = G; (e.currentTarget as HTMLDivElement).style.boxShadow = `0 0 20px rgba(226,183,105,0.2)`; }}
                onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.borderColor = ''; (e.currentTarget as HTMLDivElement).style.boxShadow = ''; }}>
                <step.icon size={18} color="rgba(255,255,255,0.38)" />
              </div>
              <span style={{ fontFamily: F, fontSize: 10, fontWeight: 700, letterSpacing: '0.3em', color: 'rgba(255,255,255,0.32)', textTransform: 'uppercase' }}>{step.label}</span>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      ══════════════════════════════════════════════════════════ */}
      <section style={{ position: 'relative', minHeight: '85dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', overflow: 'hidden', background: '#000' }}>
        {/* Dramatic center glow */}
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 800, height: 800, borderRadius: '50%', background: 'radial-gradient(circle, rgba(226,183,105,0.09) 0%, rgba(226,183,105,0.03) 40%, transparent 70%)', pointerEvents: 'none' }} />
        {/* Rings */}
        {[600, 450, 320].map((size, i) => (
          <div key={i} style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: size, height: size, borderRadius: '50%', border: `1px solid rgba(226,183,105,${0.04 - i * 0.01})`, pointerEvents: 'none' }} />
        ))}

        <div style={{ position: 'relative', zIndex: 10, maxWidth: 860, padding: '0 32px', paddingBottom: 120 }}>
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
          ...
          <motion.h2 initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            style={{ fontFamily: F, fontSize: 'clamp(44px,7vw,110px)', fontWeight: 800, lineHeight: 1.02, letterSpacing: '-0.025em', color: '#EDE8D8', margin: '0 0 52px' }}>
            {isAr ? 'ابنِ\nبذكاء' : 'Build with\nintelligence'}
          </motion.h2>
        ...
        {/* Footer bar */}
        <div className="stitch-glass-dark" style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '20px clamp(24px,5vw,80px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 20, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          <div style={{ display: 'flex', gap: 32 }}>
            {['Privacy Policy', 'Terms', 'Pricing', 'Contact'].map(link => (
              <a key={link} href="#" style={{ fontFamily: F, fontSize: 11, fontWeight: 600, letterSpacing: '0.16em', color: 'rgba(255,255,255,0.28)', textDecoration: 'none', textTransform: 'uppercase', transition: 'color 0.2s' }}
          ...
          <span style={{ fontFamily: F, fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', color: 'rgba(255,255,255,0.16)', textTransform: 'uppercase' }}>© 2026 AYN Intelligence</span>
        </div>
      </section>

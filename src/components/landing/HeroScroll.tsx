  ...
  return (
    <div style={{ background: '#000' }}>
      ...
      <section style={{ position: 'relative', minHeight: '100dvh', display: 'flex', alignItems: 'center', padding: 'clamp(72px,8vw,120px) clamp(20px,5vw,96px)', overflow: 'hidden', background: '#030200' }}>
        ...
        {/* Right: abstract geometry — desktop only, static (no JS animations) */}
        <div className="landing-decor-lg" style={{ position: 'absolute', right: '8%', top: '50%', transform: 'translateY(-50%)', width: 480, height: 480, pointerEvents: 'none' }}>
          <div style={{ position: 'absolute', inset: 0, border: '1px solid rgba(251,146,60,0.14)', borderRadius: '30% 70% 70% 30% / 30% 30% 70% 70%' }} />
          <div style={{ position: 'absolute', inset: 40, border: '1px solid rgba(251,146,60,0.10)', borderRadius: '70% 30% 30% 70% / 70% 70% 30% 30%' }} />
          ...
          {[0,60,120,180,240,300].map((deg, i) => {
            ...
            return <div key={i} style={{ position: 'absolute', width: 6, height: 6, borderRadius: '50%', background: G, left: x, top: y, opacity: 0.6, boxShadow: `0 0 8px ${G}` }} />;
          })}
        </div>

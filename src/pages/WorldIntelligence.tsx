          ...
          <AnimatePresence mode="wait">
            ...
            {activeSection === 'overview' && (
              <motion.div key="overview" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}
                ...
                <div className="rounded-3xl overflow-hidden border border-white/[0.06] shadow-[0_16px_48px_rgba(0,0,0,0.4)]" style={{ height: 'clamp(420px, 62vh, 700px)' }}>
                  <HeatMap2D
                    points={mapPoints}
            ...
            {activeSection === 'predictions' && (
              <motion.div key="predictions" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}
                ...
                {/* Graph Intelligence */}
                <section>
                  <SectionHeader icon={Network} label="AYN" title="AYN Predictions" description="High-conviction predictions generated by AYN's signal graph — updated every 4 hours." count={masterPreds.length} />

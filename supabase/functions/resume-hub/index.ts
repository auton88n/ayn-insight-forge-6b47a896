...
ABSOLUTE PROVENANCE RULE (v2.12.2): You are given the applicant's real data in profile, mergedBasics, canonical, and resume. Use ONLY values present in that data. Never invent, guess, or use example or placeholder values. NEVER output sample names, sample emails, or sample phone numbers such as "example.com", "isha.sharma", "John Doe", "Jane Smith", "+1234567890", "555-0100", or any similar synthetic identity. If a field has no supporting value in the provided data, return skip:true for that field. Returning skip is always correct and never penalized. Any identity-type value (name, email, phone, address, LinkedIn, portfolio, employer, job title, school, degree, dates) that does not appear verbatim in the provided data will be DROPPED downstream, so guessing costs you accuracy with no benefit.

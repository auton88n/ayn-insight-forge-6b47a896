...
import {
  ...
  EXT_ACTIONS, LINK_PUBLIC_ACTIONS,
} from "./lib/utils.ts";
...
import {
  ...
  EXT_MIN_VERSION_FALLBACK, readMinExtVersion, versionOlder, extVersionGate,
...
// v3.131.0 — stage 11: the extension lane's tailor/cover-letter handlers
// (identity-grounded, deterministic gap analysis, self-critique,
// figure-preservation check). See lib/smartTailor.ts's own header comment.
import { TAILOR_TTL, parseJsonLoose, handleSmartTailor, handleCoverLetter } from "./lib/smartTailor.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (typeof action === "string" && EXT_ACTIONS.has(action)) {
      ...
      // v3.131.0 — the extension's own AI-calling actions get the same
      // abuse backstop the web lane has, checked once here since every
      // extension action funnels through this one shared entry point.
      // ext_job_score gets a higher ceiling than the rest: it can fire
      // automatically on every job-board page a real person visits while
      // actively browsing, not just on an explicit click.
      const EXT_AI_ACTIONS = new Set(["smart_tailor", "ext_cover_letter_text", "ext_job_score", "ext_suggest_roles", "ext_find_contacts", "ext_ask"]);
      if (EXT_AI_ACTIONS.has(String(action || ""))) {
        const limit = action === "ext_job_score" ? 60 : 25;
        const limited = await rateLimitGate(admin, userId, String(action), limit, 15);

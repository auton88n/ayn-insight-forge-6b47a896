...
function humanizeAny(v) {
    if (v == null)
        return v;
    if (typeof v === "string")
        return humanize(v);
    if (Array.isArray(v))
        return v.map((x) => humanizeAny(x));
    if (typeof v === "object") {
        const out = {};
...
const json = (data, status = 200) => new Response(JSON.stringify(humanizeAny(data)), {
    status,
            ...
            if (r.ok) {
                ...
                if (tc) {
                    try {
                        ...
                        return { text: tc, structured: undefined };
                    }
                }
...
async function parseJobMeta(fullJd, urlHint, titleHint, companyHint) {
    if (!fullJd || fullJd.trim().length < 40)
        return EMPTY_PARSED;
...
YEARS_REQUIRED: integer if the JD says "X+ years"; omit otherwise.`,
        ...
        console.warn("parseJobMeta failed", e.message);
        return EMPTY_PARSED;
    }
}
...
function keywordFallbackScore(canonical, fullJd, parsed) {
    const userSkills = new Set();
    if (canonical) {
        for (const s of canonical.skills)
            userSkills.add(s.name.toLowerCase().trim());
        for (const t of (canonical.derived.top_skills || []))
            userSkills.add(String(t).toLowerCase().trim());
    }
...
async function loadCanonical(admin, userId) {
...
- All dates: keep the format as written ("2021", "Jan 2021", "2021-03"). Do not normalize.`,
    ...
    if (!out)
        throw new Error("Canonical extraction returned no structured output");
    return {
        skills: out.skills || [],
    ...
    try {
        ...
        if (typeof action === "string" && EXT_ACTIONS.has(action)) {
            ...
            if (action === "ext_ingest_job") {
                let { source_url, html, text, company, title, location: loc, jd_text } = payload;
                ...
                if (!company || !title || !jd_text) {
                    try {
                        ...
                        console.warn("ext parse failed", e);
                    }
                }
            ...
            if (action === "ext_autofill") {
                const { fields, jobText, jobTitle, company, ats, url } = payload;
                if (!Array.isArray(fields))
                    return json({ error: "fields required" }, 400);
                if (fields.length === 0)
                    return json({ values: [], meta: { reason: "no_form_fields" } });
                ...
                const rb = resume?.content || null;
                const basics = (rb?.basics || {});
                const work = (rb?.work || []);
                const edu = (rb?.education || []);
                ...
                for (const w of work) {
                    ...
                    if (sy)
                        yoe += Math.max(0, ey - sy);
                }
                const eduStr = edu.map(e => `${e.degree || ""} ${e.field || ""}`).join(" ").toLowerCase();
                let educationLevel = "";
                if (/ph\.?d|doctor/.test(eduStr))
                    educationLevel = "PhD";
                else if (/master|m\.?sc|m\.?a\.|mba|m\.?eng/.test(eduStr))
                    educationLevel = "Master's";
                else if (/bachelor|b\.?sc|b\.?a\.|b\.?eng|undergrad/.test(eduStr))
                    educationLevel = "Bachelor's";
                else if (/associate|diploma/.test(eduStr))
                ...
                const merged = {
                    ...
                    linkedin_url: profile?.linkedin_url || basics.links?.find?.(l => /linkedin/i.test(l?.label || l?.url || ""))?.url || "",
                    portfolio_url: profile?.portfolio_url || basics.links?.find?.(l => !/linkedin/i.test(l?.label || l?.url || ""))?.url || "",
                    ...
                    current_title: canonical?.derived?.current_title || work[0]?.title || basics.title || "",
                    current_company: canonical?.derived?.current_company || work[0]?.company || "",
...
- reasoning: ONE short sentence ("From profile.email", "Computed from 5 roles since 2019", "Matched PM bullet to JD").`,
            ...
            if (action === "ext_tailor") {
                ...
                if (!job || !resume)
                    return json({ error: "Missing job or primary resume" }, 404);
                const r = await callAI({
                    model: QUALITY_MODEL,
            ...
            if (action === "ext_cover_letter") {
                const jobId = payload.job_id;
                const tone = payload.tone || "professional, warm";
                if (!jobId)
                    return json({ error: "job_id required" }, 400);
                const [{ data: job }, { data: resume }, canonical] = await Promise.all([
            ...
            if (action === "ext_job_ingest") {
                const { url, urlHash, title, company, fullJd } = payload;
                if (!fullJd || fullJd.trim().length < 40)
                    return json({ error: "fullJd required" }, 400);
                ...
                if (error)
                    console.warn("job_cache upsert failed", error.message);
            ...
            if (action === "ext_job_score") {
                const { urlHash, url, jobTitle, company, fullJd, jobSnippet } = payload;
                ...
                if (hash) {
                    ...
                    if (c && new Date(c.expires_at).getTime() > Date.now()) {
                        cachedRow = { full_jd: c.full_jd, parsed: (c.parsed || EMPTY_PARSED), title: c.title, company: c.company };
                    }
                }
                let fullJdResolved = cachedRow?.full_jd || "";
                let parsedJob = cachedRow?.parsed || EMPTY_PARSED;
                let source = "full";
                if (!cachedRow && fullJd && fullJd.length >= 40) {
                    ...
                    admin.from("job_cache").upsert(row, { onConflict: "url_hash" }).then(({ error }) => {
                        if (error)
                            console.warn("job_cache inline-ingest upsert failed", error.message);
                    });
                }
                else if (!cachedRow) {
                ...
                const userSkillIndex = new Map();
                if (canonical) {
                    for (const s of canonical.skills) {
                        const k = s.name.toLowerCase().trim();
                        if (k)
                            userSkillIndex.set(k, s.name);
                    }
                    for (const t of (canonical.derived.top_skills || [])) {
                        const k = String(t).toLowerCase().trim();
                        if (k && !userSkillIndex.has(k))
                            userSkillIndex.set(k, String(t));
                    }
                }
...
${fullJdResolved.slice(0, 15000)}`,
                    ...
                    console.warn("ext_job_score AI failed; using keyword fallback:", e.message);
                    aiOk = false;
                }
                if (!aiOk) {
...
- Be specific to the US and Canadian job markets and their actual experience level`,
                ...
                let parsed = { roles: [], keywords: [], summary: "" };
                try {
                    const raw = r.text.replace(/\`\`\`(?:json)?\s*/gi, "").replace(/\`\`\`/g, "").trim();
                    const s = raw.indexOf("{");
                    const e = raw.lastIndexOf("}");
            ...
            if (action === "ext_find_contacts") {
                const { company, jobTitle, jobUrl, jobSnippet } = payload;
                if (!company)
                    return json({ error: "company required" }, 400);
...
CANDIDATE BACKGROUND: ${candidateBackground}`,
                ...
                return json({
                    contacts: (parsed.contacts || []).slice(0, 3),
                    emailFormats: (parsed.emailFormats || []).slice(0, 3),
            ...
            if (action === "ext_cover_letter_text") {
                const { resumeText, jdText, tone, company, jobTitle } = payload;
                if (!resumeText || !jdText)
                    return json({ error: "resumeText and jdText required" }, 400);
                const r = await callAI({
                    model: QUALITY_MODEL,
...
- Plain text, no markdown.`,
            ...
            if (action === "ext_save_application") {
                const { jobTitle, company, jobUrl, status, score, salaryEstimate, notes } = payload;
                if (!company || !jobTitle)
                    return json({ error: "company and jobTitle required" }, 400);
            ...
            if (action === "ext_download_resume_text") {
                ...
                if (!resume?.content)
                    return json({ error: "No primary resume saved in AYN" }, 404);
                ...
                const lines = [];
                if (basics.name)
                    lines.push(String(basics.name).toUpperCase());
                if (basics.title)
                    lines.push(String(basics.title));
                const contact = [basics.email, basics.phone, basics.location].filter(Boolean).join(" | ");
                if (contact)
                    lines.push(contact);
                if (basics.summary) {
                    lines.push("", "SUMMARY", String(basics.summary));
                }
                if (work.length) {
            ...
            if (action === "ext_ask") {
                const { sessionId, question, jobTitle, company, jobText, url } = payload;
                if (!question)
                    return json({ error: "question required" }, 400);
                const { data: resume } = await admin.from("resumes").select("content").eq("user_id", userId).eq("is_primary", true).maybeSingle();
                const rb = resume?.content;
                const bas = (rb?.basics || {});
                const works = (rb?.work || []);
                const skills = (rb?.skills || []);
            ...
            if (action === "smart_tailor") {
                const { resumeText, jdText, jobTitle, company } = payload;
                if (!resumeText || !jdText)
                    return json({ error: "resumeText and jdText required" }, 400);
                const r = await callAI({
                    model: QUALITY_MODEL,
...
VOICE: write bullets and changes the way a thoughtful person writes. Vary sentence length, plain natural language, no AI clichés ("leverage", "passionate", "in today's fast-paced"), no em dashes, no en dashes, never use ' - ' as a connector. Write ranges with the word 'to'.`,
                ...
                return json({
                    keywords: Array.isArray(parsed.keywords) ? parsed.keywords.slice(0, 14).map(k => ({ text: String(k.text || ""), inResume: Boolean(k.inResume), importance: String(k.importance || "medium") })) : [],
            ...
            if (action === "ext_save_answer") {
                const { questionText, answerText, company, role } = payload;
                if (!questionText || !answerText)
                    return json({ error: "questionText + answerText required" }, 400);
            ...
            if (action === "ext_lookup_answer") {
                ...
                if (!questionText)
                    return json({ error: "questionText required" }, 400);
                const normalized = questionText.toLowerCase().replace(/[^a-z0-9 ]+/g, " ").replace(/\s+/g, " ").trim().slice(0, 240);
                const hash = await sha256Hex(normalized);
            ...
            if (action === "ext_get_resume_blob") {
                const { data: resume } = await admin.from("resumes").select("content").eq("user_id", userId).eq("is_primary", true).maybeSingle();
                if (!resume?.content)
                    return json({ error: "No resume on file. Upload one at aynn.io first." }, 404);
                const rc = resume.content;
                const basics = (rc.basics || {});
                const work = (rc.work || []);
                const edu = (rc.education || []);
        ...
        if (action === "link_approve") {
            const { code, device_label } = payload;
            if (!code)
                return json({ error: "code required" }, 400);
        ...
        if (action === "token_mint") {
            const label = payload.label || "Browser";
        ...
        if (action === "parse") {
            ...
            if (!resumeText || resumeText.length > 60000)
                return json({ error: "Bad resumeText" }, 400);
            const r = await callAI({
        ...
        if (action === "parse_file") {
            ...
            if (isText) {
                try {
                    resumeText = new TextDecoder("utf-8").decode(b64ToBytes(fileBase64));
                }
                catch (_) { /* noop */ }
            }
            else if (isDocx) {
            ...
            if (!r.ok) {
                const t = await r.text();
                return json({ error: `AI error ${r.status}: ${t.slice(0, 300)}` }, 500);
            }
            const data = await r.json();
        ...
        if (action === "match") {
            const { resume, jdText } = payload;
            if (!jdText)
                return json({ error: "jdText required" }, 400);
            const r = await callAI({
...
8. Return the tailored resume in the same schema as the input.`,
        ...
        if (action === "cover_letter") {
            const { resume, jdText, tone, company } = payload;
            const r = await callAI({
        ...
        // ---------------- autofill ----------------
        if (action === "autofill") {
            const { fields, profile, resume } = payload;
            if (!Array.isArray(fields))
                return json({ error: "fields required" }, 400);
            const r = await callAI({
...
BACKGROUND: ${aboutMe.slice(0, 300) || JSON.stringify(resume?.content?.basics || {}).slice(0, 300)}`,
            ...
            return json({ contacts: (parsed.contacts || []).slice(0, 3), emailFormats: (parsed.emailFormats || []).slice(0, 3), companyDomain: parsed.companyDomain || "", coldOutreach: parsed.coldOutreach || "", subjectLine: parsed.subjectLine || "" });
        }
        ...
        if (action === "profile_canonical_extract") {
            ...
            if (!resume?.content && !profile)
                return json({ error: "No primary resume or profile to extract from" }, 404);
        ...
        if (action === "profile_canonical_save") {
            ...
            if (!canonical || typeof canonical !== "object")
                return json({ error: "canonical required" }, 400);
            const row = {
                user_id: userId,

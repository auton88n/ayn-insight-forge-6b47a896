...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // check at all, so anyone with the anon key could send any of the 9
    // branded templates below -- including a fake "Payment Successful"
    // receipt or a fake "Password Reset Requested" notice -- to any
    // address, unauthenticated. Its one real caller today (AuthModal.tsx's
    // post-signup welcome email) has no session yet at the moment it's
    // called, since this project has email confirmation on and signUp()
    // returns no session until the address is verified -- so requiring a
    // full auth check here would break the one legitimate use. Instead,
    // only 'welcome' (low-sensitivity, generic content) is allowed through
    // without proof of identity; every other template requires the actual
    // service-role key, i.e. a real server-to-server caller.
    const authHeader = req.headers.get('Authorization') || '';
    const bearerToken = authHeader.replace('Bearer ', '');
    const isServiceCall = bearerToken === supabaseServiceKey;
    if (!isServiceCall && emailType !== 'welcome') {
      return new Response(
    ...
    const emailResponse = await resend.emails.send({
      from: "AYN Team <noreply@ayn.careers>",
      to: [to],

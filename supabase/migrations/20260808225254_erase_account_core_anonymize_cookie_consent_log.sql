-- cookie_consent_log has a user_id column, so per this app's own erasure
-- rule (docs/map/blueprint.md #5, the single most repeated bug class in
-- this app's history), erase_account_core needs a matching line. Same
-- anonymize-not-delete treatment as ai_call_telemetry/error_logs/
-- security_logs/system_logs, since this table is operational/aggregate
-- data (an accept-or-reject decision and a timestamp), not personal
-- content, matching why it was never added to self_export_account either.

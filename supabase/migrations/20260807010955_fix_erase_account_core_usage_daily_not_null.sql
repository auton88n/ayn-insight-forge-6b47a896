-- erase_account_core tried to anonymize user_usage_daily the same way it
-- anonymizes ai_call_telemetry/usage_logs/error_logs/etc (SET user_id = NULL),
-- but user_usage_daily.user_id is NOT NULL, so any account with a row there
-- crashed the entire erasure -- the same shape of bug the v3.36.0 audit
-- already found once for llm_usage_logs. This table's rows are daily usage
-- counters with no other identifying content, so deleting the row (like the
-- other content tables above it in this function) is correct here, not
-- nulling a column that can't be null.
